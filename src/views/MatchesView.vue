...
// TÍTULO DINÁMICO DEL TOOLBAR
const toolbarTitle = computed(() => {
  if (step.value === 1) {
    return editedItem.value.id ? 'Editar Encuentro' : 'Programar Nuevo Encuentro'
  } else {
    return 'Volver a Datos del Partido'
  }
})
...
<template>
  <div class="h-100 d-flex flex-column">
    ...
    <v-dialog v-model="dialog" fullscreen transition="dialog-bottom-transition" :scrim="false">
      <v-card theme="dark" class="bg-black-theme">
        ...
        <v-toolbar color="transparent" class="border-bottom-subtle px-4" height="70">
          ...
          <!-- BOTÓN DE VOLVER cuando está en paso 2 -->
          <v-btn 
            icon="mdi-arrow-left" 
            @click="step = 1" 
            variant="text"
